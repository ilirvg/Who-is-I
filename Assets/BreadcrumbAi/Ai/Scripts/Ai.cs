...
namespace BreadcrumbAi{
	...
	public class Ai : MonoBehaviour {
        public static Func<int> ActiveFlaresFunc;
        ...
        private int activeFlaresNumber;
		...
		private void Ai_Controller(){
            ...
            if (_CanFollowPlayer && this.Ai_FindPlayer()) {
                ...
                if (ActiveFlaresFunc != null)
                    activeFlaresNumber = ActiveFlaresFunc();

                if (activeFlaresNumber > 0 && !_IsAir) {
                    if (Vector3.Distance(transform.position, Player.position) > followDistance || Vector3.Distance(transform.position, Player.position) <= attackDistance) {
                ...
                if (_IsMelee){ 
					if(Vector3.Distance(transform.position, Player.position) > followDistance){
                        ...
                        //here can attack the player. Shoot particles or anything 
                    }
				}
		...
		private void Ai_Wander(){
            ...
            if(_IsWandering){
	            if(Time.time > wanderNext){
					...
					float wanderX = UnityEngine.Random.Range(currentWanderPos.x - wanderDistance, currentWanderPos.x + wanderDistance);
					float wanderZ = UnityEngine.Random.Range(currentWanderPos.z - wanderDistance, currentWanderPos.z + wanderDistance);
